...
export function Hero() {
  ...
  const heroSlides = BASE_HERO_SLIDES.map((base) => {
    const copy = slideCopyById.get(base.id)
    if (!copy) return { ...base, eyebrow: "", title: "", description: "", ctaLabel: "" }
    return {
      ...base,
    ...
    <section id="home" className="relative min-h-[42vh] overflow-hidden pt-24 lg:pt-28">
      ...
      <div className="relative z-10 flex min-h-[42vh] flex-col items-center justify-center px-6 py-10 text-center md:py-12">
        ...
        <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row">
          <Link
            href={withLocaleHref(activeSlide.ctaHref, locale)}
          ...
          <a
            href={getWhatsAppHref(messages.hero.whatsappSupport)}
            target={WHATSAPP_ENABLED ? "_blank" : undefined}
            rel={WHATSAPP_ENABLED ? "noopener noreferrer" : undefined}
            aria-disabled={!WHATSAPP_ENABLED}
            title={!WHATSAPP_ENABLED ? messages.hero.whatsappUnavailableTitle : undefined}
