...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = locale === "ru" ? "Блог и уход за волосами" : "Hair Blog & Care Tips"
  const description =
    locale === "ru"
      ? "Практичные советы о наращивании, париках, уходе за трессами и выборе продуктов от CANDRA'S HAIR."
...
export default async function BlogPage() {
  ...
  const blogSchema = {
    ...
    url: absoluteUrl(localizedHref("/blog")),
    blogPost: BLOG_POSTS.map((post) => ({
      ...
      url: absoluteUrl(localizedHref(`/blog/${post.slug}`)),
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f6f3ef] pt-[102px] lg:pt-[108px]">
      ...
      <section className="w-full px-4 py-10 sm:px-6 lg:px-10 xl:px-12 2xl:px-16">
        <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-[#8a6b22]">
          {locale === "ru" ? "Журнал Candra's Hair" : "Candra&apos;s Hair Journal"}
        </p>
        <h1 className="mt-3 max-w-4xl font-serif text-[1.6rem] font-bold leading-tight text-[#141414] sm:text-[2.1rem]">
          {locale === "ru" ? "Блог, гайды и советы по уходу" : "Blog, Guides, and Hair Care Insights"}
        </h1>
        <p className="mt-4 max-w-4xl text-[10px] leading-relaxed text-[#45454b] sm:text-[11px]">
          {locale === "ru"
            ? "Узнайте, как выбирать продукты, сохранять качество волос и получать больше пользы от каждого заказа."
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="order-2 h-full p-0 lg:order-1 lg:sticky lg:top-[112px] lg:max-h-[calc(100vh-116px)] lg:overflow-hidden">
            <h2 className="text-[34px] font-semibold leading-none tracking-tight text-[#111]">
              {locale === "ru" ? "Товары" : "Our Product"}
            </h2>
            ...
            <div className="mt-4 divide-y divide-[#d8d0c7] lg:max-h-[calc(100vh-188px)] lg:overflow-y-auto lg:[scrollbar-width:none] lg:[-ms-overflow-style:none] lg:[&::-webkit-scrollbar]:hidden">
              {featuredProducts.map((product) => {
                ...
                return (
                  <Link
                    key={product.slug}
                    href={localizedHref(`/order/${product.slug}`)}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <h2 className="mt-2 text-[15px] font-semibold leading-tight text-[#111]">
                    <Link href={localizedHref(`/blog/${post.slug}`)} className="transition-colors hover:text-[#8a6b22]">
                      {post.title}
                    </Link>
                  </h2>
                  <p className="mt-2 text-[11px] text-[#66666d]">
                    {messages.blog.publishDateLabel}: {formatPublishedDate(post.publishedAt, locale)}
                  </p>
